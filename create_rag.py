...
class DesignSafeRAG:
    ...
    def extract_sections_from_markdown(self, content: str, file_info: Dict) -> List[Document]:
            ...
            if header_match:
                ...
                if current_content:
                    ...
                    if content_text:
                        ...
                        section_url = f"{file_info['url']}#{anchor}" if anchor else file_info['url']
        ...
        if current_content:
            ...
            if content_text:
                ...
                section_url = f"{file_info['url']}#{anchor}" if anchor and anchor != file_info['title'].lower().replace(' ', '-') else file_info['url']
        ...
        for page_info in nav_pages:
            if page_info['file'] and not page_info['file'].startswith('http'):
                ...
                if file_path.exists():
                    ...
                    logger.info(f"  Extracted {len(documents)} sections from {page_info['file']}")
                else:
    ...
    def scrape_website(self, start_url: Optional[str] = None) -> List[Document]:
        ...
        while to_visit:
            ...
            try:
                ...
                main_content = soup.find('div', class_='rst-content') or soup.find('main') or soup.find('article')
